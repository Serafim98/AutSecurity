            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="flex items-center mt-4 mb-10">
                        ...
                        hover:bg-green-700 hover:text-white focus:outline-none" href="{{ route('seguradora.create')}}" >Novo Registro</a>
                    </div>
                    <div class="flex">
                        <x-label for="filtro" :value="__('Filtrar por:')" />
                    </div>
                    <form action="{{ route('seguradora.search') }}" method="get">
                        <div class="flex">
                    ...
                    <div class="align-middle inline-block min-w-full shadow overflow-hidden bg-white shadow-dashboard px-8 pt-3
                        ...
                        <table class="min-w-full">
                            <caption>
                                    {{ $seguradoras->appends(['filtro' => isset($filtro) ? $filtro : '', 'pesquisa' => isset($pesquisa) ? $pesquisa : ''])->links()}}
                            </caption>
                            <thead>
                            <tr>
                            ...
                            @foreach($seguradoras as $c)
                                <tr>
                                    ...
                                    hover:bg-blue-700 hover:text-white focus:outline-none" href="{{ route('seguradora.edit', $c->id)}}" >Alterar</a>
                                        <a class="px-5 py-2 border-yellow-500 border text-yellow-500 rounded transition duration-300
                                    hover:bg-yellow-700 hover:text-white focus:outline-none" href="{{ route('seguradora.show', $c->id)}}" >Visualizar</a>
                                        <a class="px-5 py-2 border-red-500 border text-red-500 rounded transition duration-300
                                    hover:bg-red-700 hover:text-white focus:outline-none" href="{{ route('seguradora.delete', $c->id)}}" >Excluir</a>
                                    </td>
                                </tr>
                            @endforeach
